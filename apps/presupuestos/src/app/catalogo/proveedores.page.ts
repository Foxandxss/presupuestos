...
export class ProveedoresPage {
  ...
  protected readonly resumen = computed(() => {
    ...
    const visibles = this.proveedoresFiltrados().length;
  ...
  guardarInline(row: Proveedor, nuevoNombre: string): void {
  ...
  async eliminar(row: Proveedor): Promise<void> {
    const ok = await this.confirm.destructivo({
      titulo: 'Eliminar proveedor',
      mensaje: `¿Eliminar el proveedor "${row.nombre}"? Esta acción es irreversible.`,
